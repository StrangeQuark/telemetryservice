pipeline {
    ...
    environment {
        ...
        CICD_TOKEN = credentials('TELEMETRY_CICD_TOKEN')
            ...
            steps {
                script {
                    if(VAULTSERVICE_ENABLED == "true") {
                        ...
                        writeFile file: 'telemetryservice.env', text: response.content
                ...
                script {
                    ...
                    if(kubernetesEnabled) {
                        ...
                        if(imageRepository.isEmpty() || kubernetesServiceUrl.isEmpty())
                            error("TelemetryService Kubernetes deployment configuration is incomplete")
                        ...
                        withEnv(["SERVICE_IMAGE=" + image, "KUBERNETESERVICE_URL=" + kubernetesServiceUrl]) {
                            ...
                            sh '''
                                curl --fail-with-body -X POST \\
                                    ...
                                    -F "serviceName=telemetryservice" \\
                                    -F "image=$SERVICE_IMAGE" \\
                                    -F "environmentFile=@telemetryservice.env" \\
                    ...
                    try {
                        sh "docker compose --env-file telemetryservice.env up --build -d"
                        ...
                        for (int i = 0; i < maxRetries; i++) {
                            try {
                                ...
                                def healthResponse = httpRequest(
                                    url: 'http://localhost:6050/api/telemetry/health',
                                    validResponseCodes: '200'
                                )
    ...
    post {
        always {
            sh "rm -f telemetryservice.env"
            echo "Cleaned up telemetryservice.env"
        }
    }
}
